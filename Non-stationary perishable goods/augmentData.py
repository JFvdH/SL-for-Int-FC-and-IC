...
def augmentData(input_data, params, hyperparams) :
    ...
    for i in range(1, m+L_max+1) :
        nextD = np.expand_dims(np.hstack([data.iloc[i:,D_index], np.full(i, np.nan)]), 1)
        D = np.hstack([D, nextD])
    D = pd.DataFrame(D, columns=["D_"+str(i) for i in range(1, m+L_max+2)])
    ...
    for i in range(m-1) :
        ...
        perishables -= P[i,:]
    P[m-1,:] = perishables
    augmentation = pd.DataFrame(np.hstack([np.expand_dims(I,1), A, P.transpose()]),
