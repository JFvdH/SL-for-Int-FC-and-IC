...
def train(data, params, hyperparams, verbose = False) :
    ...
    m = data.iloc[0].loc['m'] # Time to perish 
    L = data.iloc[0].loc['L'] # Leadtime
    L_max = params['L_max'] # Leadtime of product with the longest leadtime
    ...
    D = copy.deepcopy(data.loc[:,['D_'+str(i) for i in range(1, m+L_max+2)]].to_numpy()).transpose().astype(int)
    A = copy.deepcopy(data.loc[:,['A_'+str(i) for i in range(1, L_max)]].to_numpy()).transpose().astype(int)
    P = copy.deepcopy(data.loc[:,['P_'+str(i) for i in range(1, m+1)]].to_numpy()).transpose().astype(int)
    ...
    X = data.drop(['demand', 'date', 'store_closed', 
                   'm', 'L'] + ['D_'+str(i) for i in range(1, m+L_max+2)], axis=1)
